...
const navItems = [
  ...
  { href: "/admin/connectors", label: "Connectors", icon: ConnectorsIcon, roles: ["OWNER", "ADMIN"] },
  { href: "/admin/workflows", label: "Workflows", icon: WorkflowIcon, roles: ["OWNER", "ADMIN"] },
  { href: "/admin/automations", label: "Automations", icon: AutomationsIcon, roles: ["OWNER", "ADMIN"] },
  { href: "/admin/settings", label: "Settings", icon: SettingsIcon, roles: ["OWNER", "ADMIN"] },
];

export function Sidebar({ userRole, userName, userEmail, forceExpanded }: SidebarProps) {
    ...
    <div className={cn(
      ...
      <div className={cn("py-2.5 border-t border-[#21262d]", isCollapsed ? "px-1.5" : "px-2")}>
        <button
          ...
          className={cn(
            "w-full rounded-lg transition-colors hover:bg-[#21262d]",
            isCollapsed ? "flex flex-col items-center gap-1 p-1.5" : "flex items-center gap-2 px-1.5 py-1.5"
          )}
          title="Profile"
        >
