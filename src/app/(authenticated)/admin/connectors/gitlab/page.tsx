...
export default function GitLabPage() {
  ...
  async function handleConnect(e: React.FormEvent) {
    ...
    setConnecting(true);
    try {
      const res = await fetch("/api/admin/gitlab/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ gitlabUrl, pat, groupId }),
      });
      ...
      toast.success(`Connected to group: ${data.group.name}`);
      setConnected(true);
      fetchProjects();
    } catch {
  ...
  async function fetchProjects() {
    ...
    try {
      const res = await fetch("/api/admin/gitlab/projects");
      if (res.ok) {
        const data = await res.json();
        setProjects(data);
        setConnected(true);
  ...
  async function toggleInclude(projectId: string, included: boolean) {
    try {
      await fetch("/api/admin/gitlab/projects", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleIndex(projectId: string) {
    ...
    try {
      const res = await fetch("/api/admin/gitlab/index", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  function startPolling() {
    const interval = setInterval(async () => {
      try {
        const res = await fetch("/api/admin/gitlab/projects?local=true");
        if (res.ok) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-foreground">GitLab Integration</h1>
        <p className="text-muted-foreground mt-1">
          Connect your GitLab group and manage repository indexing.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Provide your GitLab Personal Access Token with <code>read_api</code> scope.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleConnect} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <Label htmlFor="group-id">Group ID or Path</Label>
                <Input
                  id="group-id"
                  value={groupId}
          ...
          <CardHeader>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
              <div>
                ...
                <CardDescription>
                  Include projects for indexing, then index them to make their code available to the AI.
                </CardDescription>
              </div>
          ...
          <CardContent>
            {projects.length === 0 ? (
              <p className="text-muted-foreground text-sm">
                No projects found. Click &quot;Connect &amp; Sync&quot; to fetch projects from GitLab.
              </p>
            ) : (
